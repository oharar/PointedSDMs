...
#' @export
#' @import sp
AddDistToRangeToStack <- function(in.stk, coords, polys, scale=FALSE) {
  ...
  Locations <- SpatialPoints(in.stk$effects$data[UsePts,coords], proj4string = CRS(proj4string(polys)))

  Dists <- AddDistToRangeToSpatialPoints(data = Locations, polys = polys, scale=scale)
