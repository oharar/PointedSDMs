...
#' @import sp
#' @import INLA
AddDistToRangeToDataFrame <- function(df, coords, polynoms, name="dist", scale=FALSE) {
  ...
  #  in.stk <- stk.all; coords <- c("long", "lat"); polynoms <- PolyCoords$`iucn_birds-14034`; name <- "iucnbirds14034"
  ...
  if("package:parallel" %in% search()) {
    InPolys <- data.frame(parallel::mclapply(polynoms, function(ply, pts)
  ...
  if(sum(Dist==1)>0) {
    if("package:parallel" %in% search()) {
      dists <- parallel::mclapply(polynoms, function(ply, pts) geosphere::dist2Line(pts, ply)[,"distance"],
                                  pts=Locs[Dist==1,])
    } else {
      dists <- lapply(polynoms, function(ply, pts) geosphere::dist2Line(pts, ply)[,"distance"], pts=Locs[Dist==1,])
    }
