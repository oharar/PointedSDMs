...
Then we out the covariate data into a spatial data frame, and plot tehm
...
spplot(Covariates, layout=c(3,1), col.regions=brewer.pal(6, "Blues")[-1], key.space="right")
...
# summary(SolTinModel$model)$hyperpar
```
...
All of this is a bit disappointing - one would hope an example species would show some response to hte environment. The next step is to look at interactions, which will need some changes to FitModel(), to passbetter specify the model.
